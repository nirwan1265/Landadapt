...
gwas_cfg <- list(
  list(species = "Arabidopsis", file = "/Users/nirwantandukar/Documents/Research/results/GWAS/landadapt/arabidopsis/arabidopsis_ph.txt", sep = "\t", chr_col = "chr", pos_col = "ps", p_col = "p_wald", snp_col = "rs"),
  list(species = "Barley", file = "/Users/nirwantandukar/Documents/Research/results/GWAS/landadapt/barley/barley_ph.txt", sep = "\t", chr_col = "chr", pos_col = "ps", p_col = "p_wald", snp_col = "rs"),
  list(species = "Rice", file = "/Users/nirwantandukar/Documents/Research/results/GWAS/landadapt/rice/rice_ph_worldclim.txt", sep = "\t", chr_col = "chr", pos_col = "ps", p_col = "p_wald", snp_col = "rs"),
  list(species = "Maize", file = "/Users/nirwantandukar/Documents/Research/results/GWAS/landadapt/maize/maize_ph.txt", sep = "\t", chr_col = "chr", pos_col = "ps", p_col = "p_wald", snp_col = "rs"),
  list(species = "Sorghum", file = "/Users/nirwantandukar/Documents/Research/results/GWAS/landadapt/sorghum/sorghum_ph.txt", sep = "\t", chr_col = "chr", pos_col = "ps", p_col = "p_wald", snp_col = "rs")
)

pheno_cfg <- list(
  list(species = "Arabidopsis", file = "data/pH/arabidopsis_pH_values.csv"),
  list(species = "Barley", file = "data/pH/barley_pH_values.csv"),
  list(species = "Rice", file = "data/pH/rice_pH_values.csv"),
  list(species = "Maize", file = "data/pH/maize_pH_values.csv"),
  list(species = "Sorghum", file = "data/pH/sorghum_pH_values.csv")
)
...
write.csv(selection_counts, file.path(res_dir, "ph_top0p5_and_bonf_selection_counts.csv"), row.names = FALSE)
write.csv(top_snps, file.path(res_dir, "ph_top0p5_selected_snps.csv"), row.names = FALSE)
write.csv(bonf_snps, file.path(res_dir, "ph_bonferroni_selected_snps.csv"), row.names = FALSE)
...
write.csv(cutoffs, file.path(res_dir, "ph_top0p5_cutoffs.csv"), row.names = FALSE)
...
p_manhattan <- ggplot(prepared, aes(x = cum_pos, y = logp_plot)) +
  ...
  labs(
    title = "Manhattan Plots (pH GWAS; Top 0.5% SNP Threshold Per Species)",
...
p_pheno <- ggplot(pheno_df, aes(x = ph, fill = species)) +
  geom_histogram(bins = 35, color = "white", linewidth = 0.15) +
  facet_wrap(~ species, ncol = 1, scales = "free_y") +
  scale_fill_manual(values = fill_map) +
  labs(
    title = "Soil pH Phenotype Distribution",
    x = "pH value",
    y = "Count"
  ) +
  theme_bw(base_size = 11) +
...
fig4_out <- file.path(fig_dir, "Fig4_manhattan_plus_pH.png")
...
write.csv(top_gene_tbl, file.path(res_dir, "ph_top0p5_gene_table.csv"), row.names = FALSE)
write.csv(bonf_gene_tbl, file.path(res_dir, "ph_bonferroni_gene_table.csv"), row.names = FALSE)
...
write.csv(bonf_out, file.path(res_dir, "ph_bonferroni_top_genes_species_gene_pvalue.csv"), row.names = FALSE)

message("Saved Fig4: ", fig4_out)
message("Saved pH GWAS tables in: ", res_dir)
