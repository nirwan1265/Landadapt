...
layers <- list(
  list(
    key = "AM_roots_colonized",
    tif = file.path(predictor_dir, "AM_roots_colonized.tif"),
    out_dir = "data/AM_roots_colonized",
    value_col = "am_roots_colonized_value"
  ),
  list(
    key = "AM_rel_abundance_colonization",
    tif = file.path(predictor_dir, "AM_rel.abundance_colonization.tif"),
    out_dir = "data/AM_rel_abundance_colonization",
    value_col = "am_rel_abundance_colonization_value"
  )
)
...
for (x in layers) {
  ...
  if (!file.exists(x$tif)) stop("Missing tif: ", x$tif)
}
...
for (layer in layers) {
  ...
  message("Saved layer values: ", layer$key, " -> ", layer$out_dir)
}
